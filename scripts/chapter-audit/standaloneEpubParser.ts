/*
  STANDALONE EPUB CHAPTER PARSER

  This is a copied and isolated parser used only for calibration work.
  It exists so we can experiment on large Gutenberg corpora without touching
  the real app extractor while we are still validating behavior.

  Purpose:
  - open an EPUB from raw bytes
  - parse OPF/nav/NCX structure
  - extract readable paragraphs
  - derive chapter starts from TOC signals plus recovery heuristics
  - return a clean standalone result for audit scripts

  This file is intentionally separate from the app parser.
*/
import { load as loadHtml, type Cheerio, type CheerioAPI } from "cheerio";
import type { AnyNode } from "domhandler";
import { StandaloneZipArchive } from "./standaloneZipArchive";
import { tokenizeParagraph } from "./standaloneWordExtraction";

export type StandaloneParagraph = {
  id: number;
  text: string;
};

export type StandaloneChapter = {
  title: string;
  start_paragraph_id: number;
};

export type StandaloneEpubResult = {
  title: string;
  author: string | null;
  language: string | null;
  coverPath: string | null;
  paragraphs: StandaloneParagraph[];
...
type ParsedChapter = StandaloneChapter;

type ParagraphContext = {
  paragraph: StandaloneParagraph;
  normalized: string;
  file: string;
  spineIndex: number;
};
...
function shiftChapterStartsPastHeadings(chapters: ParsedChapter[], paragraphs: StandaloneParagraph[]): ParsedChapter[] {
...
function finalizeChapters(chapters: ParsedChapter[], paragraphs: StandaloneParagraph[], bookTitle: string): ParsedChapter[] {
...
export async function parseEpubBytes(bytes: Uint8Array): Promise<StandaloneEpubResult> {
  const zip = StandaloneZipArchive.fromBytes(bytes);
  ...
  const paragraphs: StandaloneParagraph[] = [];
  ...
  for (const [spineIndex, spinePath] of opf.spinePaths.entries()) {
    ...
    if (fileEntries.length === 0 && firstParagraphIdInFile !== null && tocEntries.length === 0) {
      const chapterTitle = extractHeadingFromChapter(loadHtml(chapterHtml)) ?? null;
      if (chapterTitle) {
  ...
  let chapters: StandaloneChapter[] = tocEntries.length > 0
