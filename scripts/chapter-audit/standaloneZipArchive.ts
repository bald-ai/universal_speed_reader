/*
  STANDALONE ZIP / EPUB ARCHIVE READER

  This file is a tiny local ZIP reader used by the standalone chapter audit
  pipeline. It exists because the standalone parser needs to read EPUB contents
  directly from bytes without depending on app-side code paths.

  Purpose:
  - read EPUB ZIP structures
  - locate entries in the central directory
  - decompress stored/deflated files
  - enforce simple size guards while reading local audit inputs

  This is helper infrastructure for the standalone audit parser only.
*/
...
export class StandaloneZipArchive {
  ...
  static fromBytes(bytes: Uint8Array): StandaloneZipArchive {
    ...
    return new StandaloneZipArchive(bytes, entries);
  }
  ...
  private lookupEntry(path: string): ZipEntry | null {
    ...
    return this.entriesByKey.get(normalized) ?? null;
  }
