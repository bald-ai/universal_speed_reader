...
export async function parsePdf(options: ParseOptions): Promise<ParserOutput> {
  ...
  if (sourcePath.length === 0) throw new Error("A PDF source path is required");
  ...
  let metadata = fallbackMetadata(sourcePath);
  ...
  try {
    const fileBytes = await beforeDeadline(readFile(sourcePath), deadline, timeoutMs);
    const data = Uint8Array.from(fileBytes);
    ...
    if (metadataResult.status === "fulfilled") {
      metadata = metadataFromPdf(metadataResult.value, document, sourcePath);
    } else {
      diagnostics.push({
        bucket: "Other",
        severity: "warning",
...
function pdfImagePointer(pageNumber: number, imageIndex: number, objectId: string): string {
  return `pdf://page/${pageNumber}/image/${imageIndex}?object=${encodeURIComponent(objectId)}`;
}
