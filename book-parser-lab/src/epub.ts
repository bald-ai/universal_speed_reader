...
export async function parseEpub(options: ParseOptions): Promise<ParserOutput> {
  ...
  try {
    sourceBytes = new Uint8Array(await readFile(options.sourcePath));
  } catch (error) {
    throw new EpubParseError("Crash", `Unable to read EPUB: ${options.sourcePath}`, {
      cause: error,
    });
  }
