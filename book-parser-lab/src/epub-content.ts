...
export async function discoverCover(state: ExtractionState): Promise<Cover | null> {
  ...
  const declaredImages = packageData.manifest.filter((item) =>
    isImageMediaType(item.mediaType, item.path ?? item.href),
  );
  if (declaredImages.length === 1) appendItem(declaredImages[0]);
...
async function coverFromWrapper(
  ...
  if (requireImageOnly && (countWords(visibleText) > 16 || candidates.length > 4)) return null;
