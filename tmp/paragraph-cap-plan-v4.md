# Plan: Hard paragraph upper bound (early reject) — v4
...
2. Create a hard diagnostic:
   ...
   - `bucket`: `"Other"` (exact; same bucket used by `totals_mismatch` / `timing_invalid`)
   - `message`: include actual count and cap, e.g. `This book has 142796 paragraphs; maximum supported is 50000.`
3. Build diagnostics via existing `deduplicateDiagnostics([...book.diagnostics, limitDiagnostic])`. Existing key is `` `${code}\0${bucket}\0${severity}\0${message}` ``; limit diagnostic is appended after pre-existing ones, so preserved diagnostics keep their relative order and the limit entry appears after them unless an identical key already exists.
4. Return `{ pass: false, diagnostics }` **without** running remaining validators.

### Safe max loop (crash fix even under the cap)
Keep the iterative max **inline** in `validateText` (no extracted helper). Prove it via an in-limit fixture that triggers the collapsed-paragraph largest-word check (so the loop path executes). No unrelated spread cleanups.
...
2. `validateParserOutput` at **50,001**:
   - one `too_many_paragraphs` failure with bucket `"Other"`
   - pre-seeded `book.diagnostics` preserved after dedupe (same key rules as existing helper)
   - no diagnostics that only later validators could produce (proves short-circuit)
3. `parseBookBytes` with a 50,001-paragraph synthetic/parsed fixture path (or validate-integrated equivalent through `parseBookBytes` if a lightweight fixture exists): **returns** (does not throw) with `too_many_paragraphs` on `book.diagnostics`.
4. Safe-max: in-limit fixture that exercises the collapsed-paragraph largest-word path (inline loop; no extracted helper).
5. Import service new-import oversize: use **behavioral spies/mocks** for every prohibited post-parse operation (`ensureTaskSourcePersisted`, metadata `patchBook`, `classifyImportDiagnostics`, chunking, cover processing, `replaceBookContent`). Also assert exact `"Book too large"` category/message and provisional row purged. Do not rely on repository-state-only checks as the sole proof.
6. Restore oversize: assert exact `"Book too large"` category/message, prior completed content remains, book not purged.
...
4. Tests + docs.
5. Verification gate: `bun run lint`, full test suite, and `bun run build`.

## Acceptance
- ≤ 50k paragraphs: success path unchanged (aside from safer max loop).
- > 50k: clean hard-fail with explicit `"Book too large"` messaging; no stack overflow; after oversize parse returns, none of these run: source persist wait, metadata patch, diagnostic classification, chunking, cover work, content replace; new-import row purged; restore keeps prior content and surfaces the same category/message.
- No open product questions left for implementer on constant ownership, gate placement, failure category, bucket, or test strategy.
