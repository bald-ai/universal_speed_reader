# Plan: Hard paragraph upper bound (early reject) — v3
...
3. Safe-max: in-limit fixture that exercises the collapsed-paragraph largest-word path (inline loop; no extracted helper).
4. Import service new-import oversize (mock/spy or repository assertions):
   - exact `ImportFailure` category `"Book too large"` and message shape
   - provisional row purged
   - after oversize parse returns, these must not run: `ensureTaskSourcePersisted`, metadata `patchBook`, `classifyImportDiagnostics`, chunking, cover processing, `replaceBookContent` / chunk-row writes
