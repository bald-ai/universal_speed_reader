...
export function chooseChapterSource(
  ...
  if (cleanFiles.length > 0) {
    diagnostics.push({
      ...
      severity: "failure",
      message: collapsedNavigation
        ? "Navigation targets collapsed and headings were unavailable; file boundaries are not trusted as chapters"
        : "Navigation TOC and headings were unavailable; file boundaries are not trusted as chapters",
    });
    return cleanFiles;
  }
  diagnostics.push({
    bucket: "Weak / missing / nonsense chapters",
    severity: "failure",
    message: "No trustworthy chapter structure was found",
  });
