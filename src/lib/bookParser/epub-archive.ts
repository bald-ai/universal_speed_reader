...
import { dirname, joinPosixPath, normalizePosixPath } from "./path.ts";
...
export function parseReference(basePath: string, rawReference: string): ParsedReference {
  ...
  const decodedPath = decodeSafeUriComponent(rawPath).replace(/\\/gu, "/");
  const baseDirectory = dirname(normalizeArchivePath(basePath));
  const joined = decodedPath.length === 0
    ? normalizeArchivePath(basePath)
    : normalizeArchivePath(
        decodedPath.startsWith("/")
          ? decodedPath.slice(1)
          : joinPosixPath(baseDirectory === "." ? "" : baseDirectory, decodedPath),
      );
...
export function normalizeArchivePath(value: string): string {
  const replaced = value.replace(/\\/gu, "/").replace(/^\/+/, "");
  const normalized = normalizePosixPath(replaced);
